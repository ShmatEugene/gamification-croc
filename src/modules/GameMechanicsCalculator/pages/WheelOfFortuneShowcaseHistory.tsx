...
const WheelOfFortuneShowcaseHistory: FC = () => {
	const [dataSource, setDataSource] = React.useState<IHistoryTableData[]>([]);
	const { wheelOfFortuneStore } = useStores();

	async function fetchWheelOfFortuneShowcaseHistory() {
		const response = await wheelOfFortuneStore.fetchWheelOfFortuneShowcaseHistory();
			...
			response.forEach(item => {
				...
				if (item.id) {
					data.push({
						...
						tickets: itemData.calculation.tickets,
						totalSpent: itemData.calculation.spent,
					});
				}
			});
			setDataSource(data);
	...
	React.useEffect(() => {
		fetchWheelOfFortuneShowcaseHistory();
	}, []);

	const handleDelete = async (id: string) => {
		const isDeleted = await wheelOfFortuneStore.deleteWheelOfFortuneShowcaseHistoryItemById(id);
		if (isDeleted) {
	...
	const columns = [
		...
		},
		{
			...
			render: (_, record: { id: string }) => (
				<Space size='middle'>
					<Link to={RouteNames.WHEEL_OF_FORTUNE_SHOWCASE_HISTORY_ITEM.split(':')[0] + record.id}>
...
export default WheelOfFortuneShowcaseHistory;
